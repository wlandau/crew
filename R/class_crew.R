...
class_crew <- R6::R6Class(
  ...
  public = list(
    ...
    store = NULL,
    #' @field classes Named list of `R6ClassGenerator`
    ...
    #' @param classes `R6ClassGenerator`
    ...
    initialize = function(
      ...
      classes = list(default = class_worker),
      workers = list()
    ) {
      self$name <- name
      self$store <- store
    ...
    validate = function() {
      ...
      self$store$validate()
      crew_assert(length(self$classes) > 0L)
      crew_assert(is.list(self$classes))
      crew_assert_named(self$classes)
      lapply(self$classes, function(x) {
