...
On the other hand, it is not always helpful to make workers so eager to scale down. For example, if you have thousands of tasks that complete almost instantly, then `seconds_idle` and `tasks_max` should be high in order to avoid overburdening the auto-scaling algorithm amid rapidly fluctuating demand. In addition, different situations need different numbers of workers. You may need far fewer worker than you might expect.

To investigate empirically, run your workflow and then look at the output from `controller$summary()`.
