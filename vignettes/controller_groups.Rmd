...
We demonstrate with a controller of fully persistent workers which always stay running and a controller of semi-transient workers which terminate after completing  four tasks. We create controller objects with names.

```{r}
persistent <- crew_controller_callr(name = "persistent")
transient <- crew_controller_callr(name = "semi-transient", tasks_max = 4L)
```
...
```{r}
group$push(name = "my task", command = sqrt(4), controller = "semi-transient")
```
...
```{r}
group$wait(controllers = "semi-transient")
group$pop()
#> # A tibble: 1 × 7
