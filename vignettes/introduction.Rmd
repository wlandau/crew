...
`rss` stands for "resident set size", and it is the total random access memory (RAM) consumed by the process, including shared libraries that may also be in use by different processes. `pid` is the process ID, and `elapsed` is the elapsed seconds since the current R process started.
