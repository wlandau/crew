...
As a side effect, methods `push()`, `pop()`, and `scale()` also launch workers to run the tasks. However, launches may not occur immediately because `crew` may wait to collect many tasks before scaling to the appropriate number of workers. So you may need to loop over `pop()` or `scale()` multiple times to make sure enough workers are always available.
