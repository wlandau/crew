...
`crew` unavoidably launches and terminates workers one at a time. Sometimes, these operations are not instantaneous. For example, in AWS Batch, the HTTP API call to launch a worker takes about a second to run. This latency becomes burdensome when it compounds over hundreds of workers.

Fortunately, `crew` launchers can run quick local commands asynchronously. As a launcher plugin developer, all you need to do is:
...
2. Execute launches and terminations inside `self$async$eval()`.
