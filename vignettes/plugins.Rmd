...
Every `launch_worker()` method must accept arguments `call` and `name`. The former is a text string with an R function call to  [`crew_worker()`](https://wlandau.github.io/crew/reference/crew_worker.html), and `name` is a text string with a unique informative name for the worker instance. `launch_worker()` must launch a worker that runs the R code in `call`, but `name` is purely for convenience.
...
When you are done, terminate the controller. This severs the underlying network connections of the controller, which terminates the workers and dispatcher.
...
Depending on the launcher plugin, worker launches can be time-consuming. For example, each HTTP request to AWS Batch can take a couple seconds, and this latency becomes burdensome when it there are hundreds of workers. Fortunately, `crew` launchers can run launches asynchronously. As a launcher plugin developer, all you need to do is:
...
2. Execute launches inside `self$async$eval()`, and return the resulting value from `launch_worker()`.
...
Here is what the launcher class looks like. We work with `processx` PIDs directly because they are light and easy to send to local async `mirai` daemons. The `self$async$eval()` function accepts R code, data, and packages to run a quick local asynchronous task, and it returns a `mirai::mirai()` task object.
