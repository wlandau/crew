...
`crew` unavoidably launches and terminates workers one at a time. Sometimes, these operations are not instantaneous. For example, in AWS Batch, the HTTP API call to launch a worker takes about a second to run. This overhead becomes burdensome for pipelines with hundreds of parallel workers.
