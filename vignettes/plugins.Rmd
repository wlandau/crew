...
**Note:** handles in the `launches` data frame exist only for
internal auto-scaling logic and testing purposes. In long-running
or high-throughput pipelines, `crew` periodically prunes old rows from
`launches` as workers connect and disconnect, so handles are
not guaranteed to remain available for the full lifetime of a pipeline.
