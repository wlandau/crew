...
In the `launch_worker()` method above, the `call <- self$call(socket, host, port, token, name)` command creates a call to [`crew_worker()`](https://wlandau.github.io/crew/reference/crew_worker.html). The `call` object is a simple text string with R code. To see what it looks like, you can try the method in a `callr` launcher object.
...
The `processx::process$new(command = "R", args = c("-e", call))` line runs the [`crew_worker()`](https://wlandau.github.io/crew/reference/crew_worker.html) call as an external R process. This process runs in the background, connects back to `crew` and `mirai` over the local network, and accept the tasks you push to the controller. `processx::process$new()` also returns a handle which the `terminate_worker()` method can use to force-terminate the process if appropriate. `mirai` has its own way to terminate workers, so a `terminate_worker()` method is not strictly required, but it is a useful safeguard in case of awkwardly timed crashes.
