...
The [Shiny app vignette](https://wlandau.github.io/crew/articles/shiny.html) shows an approach to asynchronous Shiny apps which is straightforward to understand and implement. However, the example app is not actually entirely asynchronous. After each background task finishes, the R code goes out of its way to retrieve the results and render the plot synchronously. What if the app code could submit the task and then forget about it, trusting the plot to take care of itself automatically whenever the R session has a free moment? The [`promises`](https://rstudio.github.io/promises/) package provides this near-magic style of asynchronous programming, which has been battle-tested in JavaScript for years.
