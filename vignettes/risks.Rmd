...
2. A local [`mirai`](https://github.com/shikokuchuo/mirai) dispatcher process to schedule the tasks. If `x` is a `crew` controller, a `ps::ps_handle()` process handle of the dispatcher is retained in `x$client$dispatcher`.
