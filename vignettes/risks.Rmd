...
2. When you create a `crew` controller, create a TLS configuration object with `crew_tls()` using the following arguments:
  ...
  * `certificates`: Character vector of file paths to certificates. If the CA signed the certificate directly, then only your own certificate is needed. But for extra security, you could also supply the entire certificate chain, beginning with your certificate and ending with the certificate of the CA.
