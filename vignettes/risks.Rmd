...
2. The [`mirai`](https://github.com/shikokuchuo/mirai) dispatcher, an R process which send tasks to workers and retrieves results back. If `x` is a `crew` controller, a `ps::ps_handle()` process handle of the dispatcher is retained in `x$client$dispatcher`.
