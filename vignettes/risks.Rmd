...
In the event of a poorly-timed crash or network error, these processes may not terminate properly. If that happens, they will continue to run, which may strain traditional clusters or incur heavy expenses on the cloud. Please monitor the platforms you use and manually terminate defunct hanging processes as needed. For example, in the case of AWS Batch, [`crew.aws.batch`](https://wlandau.github.io/crew.aws.batch/) supports its own job monitoring utilities: <https://wlandau.github.io/crew.aws.batch/index.html#job-management>.
...
* Connections: each worker must run on the local network, and it must be able to dial into the local controlling R process over TCP. If these connections are blocked, please ensure there is no firewall between the workers and local process. For AWS Batch, we recommend you run the controlling R process and all the workers in a security group that points to itself (details: <https://wlandau.github.io/crew.aws.batch/#prerequisites>). You may have to discuss with your system administrator.
