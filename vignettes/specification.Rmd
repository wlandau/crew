...
Files may exist locally or on the cloud, depending on the subclass. The directory structure of a data store has a single root folder and subdirectories `inpu/` and `output/`. The former contains instructions and data for submitted jobs, and the latter contains job output written by the workers. In both cases, there is one file per job, and the file names are in aphabetical order according to the file system. Benefits of this design include:

* Polling by design: to poll, a worker object can simply check if the `output/` file exists for the job currently running.
